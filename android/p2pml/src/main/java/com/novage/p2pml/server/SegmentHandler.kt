...
//        decodedSegmentUrl = decodedSegmentUrl.replace("%25", "%")
        ...
        try {
            ...
            Logger.e(
                TAG,
                "Segment $decodedSegmentUrl not found: ${e.message}, falling back to direct fetch."
            )
            fetchAndRespondWithSegment(call, decodedSegmentUrl, byteRange)
        } catch (e: Exception) {
            Logger.e(
                TAG,
                "Error handling segment $decodedSegmentUrl request: ${e.message}, falling back to direct fetch.",
                e
            )
            fetchAndRespondWithSegment(call, decodedSegmentUrl, byteRange)
        }
